...
function animate()
{
	...
	var info = document.getElementById("info");
 	...
 	if(new Date().getTime() - start > delay * 1000){
      info.textContent = "====== FPS AVG : " + fpsmeter.fpsAvg() + " ====== / ====== FPS     : " + fpsmeter.fps() + " ======";
      return;
  }

  fpsmeter.update();

  var fps = fpsmeter.fps();
  if (isNaN(fps))
    fps = fpsmeter.fps();

  if (fps > 0) {
      info.textContent = "FPS AVG : " + fpsmeter.fpsAvg() + " / FPS : " + fps ;
  }
